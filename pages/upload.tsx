    ...
    <div className="bg-gray-200 w-full">
      <div className={`h-screen flex justify-center items-center`}>
        <Dropzone onDrop={(acceptedFiles) => setFiles(acceptedFiles)}>
          {({ getRootProps, getInputProps }) => (
            <>
              <div
                className={`m-auto relative w-1/2 h-2/3 border-2 py-10  border-black border-dashed bg-white flex justify-around flex-col items-center cursor-pointer`}{...getRootProps()}>
                <input {...getInputProps()} />
                {files.length === 0 ?
                  <div className="flex justify-around flex-col items-center h-full">
                    <svg className="mr-4" width="80" height="70" viewBox="0 0 107 93" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="flex justify-center flex-col space-y-2">
                      <p className="text-center text-2xl font-bold">파일 업로드</p>
                      <p className="text-gray-500">업로드할 파일을 끌어다놓으세요.</p>
                    </div>
                    <div className="flex justify-center flex-col space-y-2">
                      <p className="text-center">파일 포맷은 <span className="text-google-blue font-semibold">FBX, OBJ, STL, PLY</span> 등을 지원합니다.</p>
                      <p className="text-center">또한 <span className="text-google-blue font-semibold">ZIP, RAR</span> 등의 텍스처, 메쉬가 포함된 파일을 업로드 할 수 있습니다.</p>
                    </div>
                  </div>
                  :
                  <div className="w-1/2">
                    <div className={'grid grid-cols-2 text-center border border-black divide-x divide-black'}><p>name</p><p>size</p></div>
                    {files?.map((files, i) => {
      ...
      <div className={`h-screen flex space-x-10 px-20  pt-12`}>
        <div className="w-1/3 mt-16 space-y-3">
          <div className="border border-black bg-white w-2/3 h-1/3">thumbnail?</div>
          <p>가나다라.obj</p>
          <p>size</p>
        </div>
